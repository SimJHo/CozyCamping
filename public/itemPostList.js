...
window.onload = function () {
    if(id == "tent"){
        axios({
            method: 'get',
            url: 'http://localhost:3000/productlist/tent',
            responseType: 'json'
        })
        .then(function (response){
            listLength(response);
            $('.listName').remove();
            $('.recommend').append('<div class="listName"><hr>텐트<hr></div>');
            $('aside').append('<img src="../imge/제품창사진.png" width="100%">');
        })
    }
    else if(id == "chair"){
        axios({
            method: 'get',
            url: 'http://localhost:3000/productlist/chair',
            responseType: 'json'
        })
        .then(function (response){
            listLength(response);
            $('.listName').remove();
            $('.recommend').append('<div class="listName"><hr>의자<hr></div>');
            $('aside').append('<img src="../imge/제품창사진3.jpg" width="100%">');
        })
    }
    else if(id == "grill"){
        axios({
            method: 'get',
            url: 'http://localhost:3000/productlist/grill',
            responseType: 'json'
        })
        .then(function (response){
            listLength(response);
            $('.listName').remove();
            $('.recommend').append('<div class="listName"><hr>그릴<hr></div>');
            $('aside').append('<img src="../imge/제품창사진2.jpg" width="100%">');
        })
    }
    else if(id == "lantern"){
        axios({
            method: 'get',
            url: 'http://localhost:3000/productlist/lantern',
            responseType: 'json'
        })
        .then(function (response){
            listLength(response);
            $('.listName').remove();
            $('.recommend').append('<div class="listName"><hr>랜턴<hr></div>');
            $('aside').append('<img src="../imge/제품창사진5.jpg" width="100%">');
        })
    }
    ...
    if (pageCurrent == -1) { // 이전 페이지
        ...
        if(id == "tent"){
            if($('.radio1').is(":checked") == true){
                sort = "http://localhost:3000/productlist/tent";
            }
            else if($('.radio2').is(":checked") == true){
                sort = "http://localhost:3000/popularity/tent";
            }
            else if($('.radio3').is(":checked") == true){
                sort = "http://localhost:3000/lowprice/tent";
            }
            else{
                sort = "http://localhost:3000/highprice/tent";
            }
        }
        else if(id == "chair"){
            if($('.radio1').is(":checked") == true){
                sort = "http://localhost:3000/productlist/chair";
            }
            else if($('.radio2').is(":checked") == true){
                sort = "http://localhost:3000/popularity/chair";
            }
            else if($('.radio3').is(":checked") == true){
                sort = "http://localhost:3000/lowprice/chair";
            }
            else{
                sort = "http://localhost:3000/highprice/chair";
            }
        }
        else if(id == "grill"){
            if($('.radio1').is(":checked") == true){
                sort = "http://localhost:3000/productlist/grill";
            }
            else if($('.radio2').is(":checked") == true){
                sort = "http://localhost:3000/popularity/grill";
            }
            else if($('.radio3').is(":checked") == true){
                sort = "http://localhost:3000/lowprice/grill";
            }
            else{
                sort = "http://localhost:3000/highprice/grill";
            }
        }
        else if(id == "lantern"){
            if($('.radio1').is(":checked") == true){
                sort = "http://localhost:3000/productlist/lantern";
            }
            else if($('.radio2').is(":checked") == true){
                sort = "http://localhost:3000/popularity/lantern";
            }
            else if($('.radio3').is(":checked") == true){
                sort = "http://localhost:3000/lowprice/lantern";
            }
            else{
                sort = "http://localhost:3000/highprice/lantern";
            }
        }
        axios({
            method: 'get',
